...
public class MoveGenerator {
    ...
    **/
    ...
    private void generate(int offset, int row, int column, Word word,
                          Rack rack, Arc arc, List<Move> moves) {
        if (board.isOccupiedByLetter(row, column + offset)) {
            char letter = board.getField(row, column + offset);
            goOn(offset, row, column, letter, word, rack, arc.getNextArc(letter), arc, moves, false);
        }
        else if (!rack.isEmpty()) {
            for (char letter : rack.getAllowedLetters(crossSetCalculator.getCrossSet(row, column + offset), alphabet)) {
                goOn(offset, row, column, letter, word, rack.withRemovedLetter(letter), arc.getNextArc(letter), arc, moves, false);
            }
            if (rack.contains(Rack.BLANK)) {
                for (char letter : alphabet.getAllowedLetters(crossSetCalculator.getCrossSet(row, column + offset))) {
                    goOn(offset, row, column, letter, word, rack.withRemovedLetter(Rack.BLANK), arc.getNextArc(letter), arc, moves, true);
                }
            }
        }
    }
    ...
    private void goOn(int offset, int row, int column, char letter, Word word, Rack rack,
                      Arc newArc, Arc oldArc, List<Move> moves, boolean isBlankLetter) {
        if (offset <= 0) {
